...
// ✅ FUNÇÕES DE FORMATAÇÃO MONETÁRIA (REPLICADAS DO GERENTE)
...
export default function PedidosEntreguesAdmin() {
  ...
  /**
   * ✅ ATUALIZADO: Atualiza AMBOS os campos (data E frete) dos pedidos selecionados
   * - Se data vazia: remove data dos pedidos selecionados
   * - Se frete zero: marca status como pendente
   * - Se ambos zerados: limpa todos os dados
   */
  const atualizarPedidos = async () => {
    ...
    // Permitir data em branco para REMOVER datas
    ...
    try {
      const updates = Array.from(pedidosSelecionados).map(async (id) => {
        ...
        // ✅ NOVO: Pegar frete do campo input do card (valor atual editado)
        const fretePago = parseFloat(pedido?.frete_pago) || 0.0;
        
        // ✅ CORREÇÃO: status_pagamento é BOOLEAN baseado apenas no frete (como no código antigo)
        ...
        if (error) {
          console.error(`❌ Erro pedido ${id}:`, error);
          throw error;
        }
      });
      ...
      // Feedback baseado no que foi atualizado
      let mensagem = `✅ ${pedidosSelecionados.size} pedido(s) atualizado(s):`;
      
      if (dataFormatada && !dataPagamento) {
        mensagem += `\n🗑️ Data removida`;
      ...
      mensagem += `\n💰 Frete atualizado conforme valor nos cards`;
      
      alert(mensagem);
      
      // ✅ IMPORTANTE: Atualizar estado local para sincronizar com DB
  ...
  /**
   * ✅ ATUALIZADO: Atualiza valor do frete com formatação brasileira
   */
  ...
  // ============================================================================
  // ✅ FUNÇÕES DE MANIPULAÇÃO DO FRETE (REPLICADAS DO GERENTE)
  // ============================================================================
  ...
  return (
    <div className="bg-gray-50 min-h-screen mobile-optimized">
      ...
      <div className="bg-white shadow-md rounded-lg p-3 mb-3 sticky top-2 z-10 mobile-header">
        ...
        {/* ✅ ATUALIZADO: Botão sempre "Atualizar" - edita data E frete */}
        <div className="flex gap-2 items-center">
          <div className="flex-grow">
            <input
              ...
              className="w-full p-2 border border-gray-300 rounded mobile-field"
              placeholder="Deixar vazio para remover data"
            />
            <p className="text-xs text-gray-500 mt-1">
              💡 Atualiza: Data (acima) + Frete Pago (nos cards selecionados)
            </p>
          </div>
          <button 
