...
/**
 * 🚀 ENVIAR NOTIFICAÇÃO TELEGRAM POR LOJA
 * UMA notificação por loja via canal Telegram
 * Escalável para milhares de usuários por canal
 */
async function enviarNotificacaoTelegramPorLoja(pedidoInserido) {
  try {
    console.log(`📱 Enviando notificação Telegram para loja ${pedidoInserido.id_loja}...`);
    
    const notificationResponse = await fetch(`${supabaseUrl}/functions/v1/send-telegram`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        loja_id: pedidoInserido.id_loja
      })
    });

    const result = await notificationResponse.json();
    console.log('✅ Resposta Telegram por Loja:', result);
    
    if (!result.success) {
      console.warn('⚠️ Telegram canal não enviado:', result.error);
    } else {
      console.log(`✅ Canal da loja ${pedidoInserido.loja_nome} notificado!`);
    }
  } catch (error) {
    console.error('⚠️ Erro na notificação Telegram por loja:', error.message);
  }
}
  ...
  try {
    ...
    const pedidoData = {
      ...
      endereco_entrega: payload.shipping?.address_1 ? 
        ...
        null,
      // ✅ PRESERVA FORMATO ESPECIAL DOS PRODUTOS
    ...
    // ✅ ENVIAR NOTIFICAÇÃO TELEGRAM POR LOJA
    await enviarNotificacaoTelegramPorLoja(pedidoInserido);
